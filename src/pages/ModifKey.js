...
function ModifKey({setTitle}){
    ...
    <div className='Main'>
            <div className='lien'>
                <p className='Navbar'><Link to='/Homepage'>Copropriétés</Link> &gt;<Link to={`/Copropriete/${IdCopropriete}`}>{copropriete.Numero}</Link> &gt; <Link to={`/Copropriete/${IdCopropriete}/CommonKey/${IdKey}`} state={{copropriete}}>{key.name}</Link> &gt; Modif clé </p>
            </div>
            <form onSubmit={PutKey}>
                <label>Numéro de trousseau :
                    <input type='number' id='name' name='name' defaultValue={key.name} required="required"/>
                </label>
                <label>Accès :
                    <input type='text' id='acces' name='acces' defaultValue={key.acces} required="required" maxLength="36"/>
                </label>
        ...
        (<h1>Modification enregistrée <Link to={`/Copropriete/${IdCopropriete}/CommonKey/${IdKey}`} state={{copropriete}}>Page de la clé</Link></h1>)
    )
    )
}
export default ModifKey;
