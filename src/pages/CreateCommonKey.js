...
function CreateCommonKey({setTitle}){
    useEffect(()=>{
        setTitle(<h1>Nouvelle clé commune</h1>)
    },[])
    ...
    let postCommonKey = async(e )=>{
        e.preventDefault()
        let formData = new FormData();
        formData.append('name', e.target.name.value);
        formData.append('acces', e.target.acces.value);
        formData.append('image', e.target.image.files[0]); // Notez l'utilisation de .files[0]
    ...
    return(
        <div className='Main'>
            ...
            <form onSubmit={postCommonKey}>
                <label>Numéro de trousseau :
                    <input type='number' id='name' name='name' placeholder='Nom' required="required"/>
                </label>
                <label>Accès :
                    <input type='text' id='acces' name='acces' placeholder='Accès' required="required" maxLength="36"/>
                </label>
