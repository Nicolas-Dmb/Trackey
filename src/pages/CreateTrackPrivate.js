...
function CreateTrackPrivate({setTitle}){
    ...
    let getTrack = async() =>{
        let response = await fetch(`https://www.apitrackey.fr/api/TrackP/update/${IdKey}/`,{
            method:'GET',
            headers:{
            ...
            if (response.status === 202){
                navigate(`/Copropriete/PrivateKey/Returntrack/${IdKey}/`)
    ...
    let getAgency= async() =>{
        let response = await fetch(`https://www.apitrackey.fr/api/getPkey/${IdKey}/`,{
            method:'GET',
            headers:{
                'Content-Type':'application/json',}
        });
    ...
    let CreateTrack = async(e ) =>{
        e.preventDefault();
        let response = await fetch(`https://www.apitrackey.fr/api/TrackPrivate/`,{
            method:'POST',
            headers:{
                'Content-Type':'application/json',
                'Authorization': `Bearer ${authTokens.access}`},
...
export default CreateTrackPrivate;
